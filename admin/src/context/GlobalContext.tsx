...
interface GlobalContextProps {
  ...
  settings: Settings;
  branding: { logoUrl: string; logoCollapsedUrl: string; appTitle: string; faviconUrl: string };
...
export function GlobalProvider({ children }: { children: ReactNode }) {
  ...
  const [branding, setBranding] = useState<{ logoUrl: string; logoCollapsedUrl: string; appTitle: string; faviconUrl: string }>({
    logoUrl: '',
    logoCollapsedUrl: '',
    appTitle: 'FCC Maps',
    faviconUrl: '',
  ...
  }, [branding.appTitle, branding.faviconUrl]);
  ...
  const fetchData = async (mapId: string) => {
    ...
    try {
      ...
      if (settingsRes.ok) {
        ...
        // Extract and set branding properties when default settings are retrieved
        if (mapId === 'default') {
          setBranding({
            logoUrl: settingsData.logoUrl || '',
            logoCollapsedUrl: settingsData.logoCollapsedUrl || '',
            appTitle: settingsData.appTitle || 'FCC Maps',
  ...
  return (
    <GlobalContext.Provider value={{
      ...
      settings, branding, status, logs, taxonomies, colors, setColors,
